...
function AbacusCtrl(type) {
  ...
  this.activated = function (nodeId) {
    ...
    // Change offset according to top or bottom bead selected
    if (this.nodes[nodeId].position[1] >= 100) {
      var offset = dir * -1 * this.beadHeight - 60;
      if (active) offset = dir * this.beadHeight + 60;
    } else {
      var offset = dir * -1 * this.beadHeight;
      if (active) offset = dir * this.beadHeight;
    }
...
function Abacus(parentDivId, type) {
  ...
  var beadColor = "yellow";
  var topBeadColor = "mediumturquoise";
  var hooveredBeadColor = "blue";
  ...
  this.init = function () {
    ...
    canvas.width = 80 + abacusCtrl.beadLines * abacusCtrl.beadSpacing;
    canvas.height = 100 + (abacusCtrl.beadPerLine + 2) * abacusCtrl.beadHeight;
    ...
    canvas.addEventListener(
      ...
      function (event) {
        ...
        for (var i = 0; i < touches.length; i++) {
          var pos = {
            x: touches[i].pageX - canvas.offsetLeft,
            y: touches[i].pageY - canvas.offsetTop,
          };
  ...
  this.resetAbacus = function () {
    abacusCtrl.init(); // Reset the abacus controller
    that.update(); // Update the UI
  };

  var resetButton = document.createElement("button");
  resetButton.innerText = "Reset";

  resetButton.style.fontSize = "18px"//Increase font size
  resetButton.style.padding ="8px 10px" //Increase padding for a larger button
  resetButton.addEventListener("click", function () {
    that.resetAbacus(); // Call the resetAbacus method
  });
  ...
  function drawBead(nodeId, ctx) {
    ...
    if (abacusCtrl.beadPerLine == 5) {
      if (nodeId % abacusCtrl.beadPerLine == abacusCtrl.beadPerLine - 1)
        ctx.fillStyle = topBeadColor;
    } else if (abacusCtrl.beadPerLine == 7) {
      if (
        nodeId % abacusCtrl.beadPerLine == abacusCtrl.beadPerLine - 1 ||
        nodeId % abacusCtrl.beadPerLine == abacusCtrl.beadPerLine - 2
      ) {
        ctx.fillStyle = topBeadColor;
      }
    }
  ...
  this.update = function () {
    ...
    for (var i = 0; i < abacusCtrl.beadLines; i++) {
      ...
      if (abacusCtrl.type === 0) {
        valueSting = valueSum.toString(15);
      } else {
